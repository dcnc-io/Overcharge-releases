...
이번 공개 베타는 실행 검증 미완료를 명시하고 게시합니다. Windows 수신기 소스 검증에서는 포트 충돌 복구·인증 데이터 수신·같은 포트로 재시작을 확인했습니다. 소스 테스트와 패키징 검증은 실제 EXE 실행·설치·업데이트·제거 검증을 대신하지 않습니다. 실게임 수신도 이번 빌드에서 확인하지 않았습니다.

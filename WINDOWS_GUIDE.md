...
Windows 소스 검증에서 실제 WebGL과 Io 전환, 포트 복구, 인증된 합성 데이터의 영웅 표시를 확인했습니다. 최종 EXE 검증 결과는 release.json에 별도로 기록합니다. 전체 설치·업데이트·제거와 실제 봇전 픽 인식은 확인되지 않은 항목을 명시합니다. 연결 및 합성 입력 성공을 실게임 인식 성공으로 해석하지 마세요.
...
connector.14는 수신된 내 영웅을 You 카드로 표시합니다. 팀 전체 데이터가 없다고 내 영웅을 숨기지 않으며, 전달되지 않은 팀 슬롯은 추측하지 않습니다. 연결된 상태에서도 메뉴의 heartbeat에는 영웅·경기 필드가 없을 수 있습니다. 이 경우 연결됨과 경기 데이터 없음 상태를 구분합니다.

## 수신기 포트 충돌 복구

connector.13부터 사용 중인 포트에서는 운영체제가 선택한 빈 로컬 포트로 한 번 복구하고 성공한 포트를 저장합니다. Connection settings에서 실제 수신 포트를 확인하십시오. Retry receiver로 복구한 뒤 Install / repair connection을 선택해 Dota 연결 파일을 갱신합니다. 이미 실행 중인 Dota가 이전 포트를 쓰는 경우에는 사용자가 편한 시점에 게임을 재시작해야 합니다. 다른 프로그램을 종료하거나 방화벽·보안 기능을 끄지 않습니다. “Desktop ready”는 앱 화면 연결, “Listening”은 수신 준비, “Receiving game updates”는 데이터 도착을 의미합니다.

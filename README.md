...
Download the **0.1.0-connector.22** installer or portable from [overcharge.io/download](https://overcharge.io/download/). Windows x64; unsigned; manual updates. This public repository contains binaries, release metadata and documentation. Product source remains private.
...
Connector.21 adds conditional threats, ally/combo guidance for all127 catalog heroes, historical same-hero purchase references from20 recorded matches, and three explicitly illustrative team-plan images. Historical medians and authored alternatives remain separate from live inventory and current-meta claims. Existing overlay layout and connection/authentication code are preserved.

Connector.22 preserves the current focused or manually revealed match view when Dota process detection recovers during an already active match.

**Verification has limits.** Source tests and Windows offline-frame recognition passed. Exact final Windows smoke results are recorded in the release manifest; full installation/upgrade/removal, final live-game acceptance and real Steam account sign-in remain unverified. Read the [release notes](RELEASE_NOTES.md) and [release manifest](release.json) for exact evidence. Keep Windows security protections enabled; unsigned applications can be blocked by device policy.
